...
Logs are written to <log_dir>/mcp_acp_extended_logs/debug/client_wire.jsonl using Pydantic models.
...
from mcp_acp_extended.utils.logging.logging_context import (
    get_request_id,
    get_session_id,
)
from mcp_acp_extended.utils.logging.logging_helpers import categorize_error
...
class BidirectionalClientLoggingMiddleware(BaseLoggingMiddleware):
    ...
    def __init__(
        ...
        *,
        ...
        self.logger = logger or logging.getLogger("mcp-acp-extended.debug.client")
...
def setup_client_wire_logger(log_path: Path) -> logging.Logger:
    ...
    Args:
        log_path: Path to the client wire log file (e.g., <log_dir>/mcp_acp_extended_logs/debug/client_wire.jsonl).

    Returns:
        logging.Logger: Configured logger instance for client wire logs.
    """
    return setup_jsonl_logger(
        "mcp-acp-extended.debug.client",
        log_path,
        logging.INFO,
    )
...
def create_client_logging_middleware(
    ...
    if debug_enabled:
        ...
        logger = logging.getLogger("mcp-acp-extended.debug.client.null")
