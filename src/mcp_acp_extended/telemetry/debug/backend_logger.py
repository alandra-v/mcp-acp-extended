...
Logs are written to <log_dir>/mcp_acp_extended_logs/debug/backend_wire.jsonl.
...
from mcp_acp_extended.utils.logging.extractors import extract_tool_metadata
...
def setup_backend_wire_logger(log_path: Path) -> logging.Logger:
    ...
    Args:
        log_path: Path to the backend wire log file (e.g., <log_dir>/mcp_acp_extended_logs/debug/backend_wire.jsonl).

    Returns:
        logging.Logger: Configured logger instance for backend wire logs.
    """
    return setup_jsonl_logger(
        "mcp-acp-extended.debug.backend",
        log_path,
        logging.INFO,
    )
