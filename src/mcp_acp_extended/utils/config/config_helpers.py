...
def get_config_path() -> Path:
    ...
    Returns:
        Path to mcp_acp_extended_config.json in the config directory.
    """
    return get_config_dir() / "mcp_acp_extended_config.json"


def get_log_dir(config: "AppConfig") -> Path:
    """Get log directory (always <config.logging.log_dir>/mcp_acp_extended_logs/).

    Args:
        config: Application configuration.

    Returns:
        Path: Log directory path (<log_dir>/mcp_acp_extended_logs/).
    """
...
def ensure_directories(config: "AppConfig") -> None:
    ...
    log_dir = log_base / "mcp_acp_extended_logs"
