...
mcp-acp-extended config show --json    # Machine-readable format
...
mcp-acp-extended config validate --path /path/to/config.json  # Validate alternate file
```
...
**Files**:
- `mcp_acp_extended_config.json` - operational settings (auth, logging, backend, proxy, HITL)
- `policy.json` - security policies (rules only; HITL settings are in config)

**Log directory**: User-specified via `--log-dir` during init. Logs are stored in a `mcp_acp_extended_logs/` subdirectory:
...
| Field | Description |
|-------|-------------|
| `log_dir` | Base directory for logs (required, logs stored in `mcp_acp_extended_logs/` subdirectory) |
...
Human-in-the-Loop settings are configured in `mcp_acp_extended_config.json` (not policy.json).
...
**Note:** HITL settings (timeout, caching) are in `mcp_acp_extended_config.json`, not `policy.json`.
