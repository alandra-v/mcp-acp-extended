...
The web UI is an **optional add-on** for users who prefer a graphical interface for monitoring and approvals. It opens an HTTP server on port 8765, but is hardened with Host header validation, Origin checks, CSRF protection, and token authentication. Modern browsers also block cross-origin requests to localhost via Private Network Access. For minimal attack surface, you can disable the UI entirely.
...
To run without the HTTP server:
