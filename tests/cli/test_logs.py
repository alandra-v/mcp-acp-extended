    ...
    def test_list_shows_available_types(self, runner: CliRunner, mock_config: MagicMock):
        ...
        with runner.isolated_filesystem() as tmpdir:
            with patch(
                ...
                with patch(
                    "mcp_acp_extended.cli.commands.logs.get_decisions_log_path",
                    return_value=Path(tmpdir) / "decisions.jsonl",
                ):
                    with patch(
                        "mcp_acp_extended.cli.commands.logs.get_audit_log_path",
                        return_value=Path(tmpdir) / "operations.jsonl",
                    ):
                        with patch(
                            "mcp_acp_extended.cli.commands.logs.get_auth_log_path",
                            return_value=Path(tmpdir) / "auth.jsonl",
                        ):
                            with patch(
                                "mcp_acp_extended.cli.commands.logs.get_system_log_path",
