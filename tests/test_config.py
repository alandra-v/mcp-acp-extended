...
class TestAppConfig:
    ...
    def test_defaults_proxy_name(self, valid_auth_config: AuthConfig):
        ...
        # Assert
        assert config.proxy.name == "mcp-acp-extended"
...
class TestConfigHelpers:
    ...
    def test_get_log_dir_appends_mcp_acp_extended_logs(self, valid_config_dict: dict):
        # Arrange
        from mcp_acp_extended.utils.config import get_log_dir
        ...
        # Assert
        assert result.name == "mcp_acp_extended_logs"
    ...
    def test_log_path_helpers_return_correct_filename(
        ...
        # Arrange
        from mcp_acp_extended.utils import config as config_module
    ...
    def test_checksum_is_deterministic(self, tmp_path: Path):
        # Arrange
        from mcp_acp_extended.utils.config import compute_config_checksum
    ...
    def test_ensure_directories_creates_log_structure_info_level(
        ...
        from mcp_acp_extended.utils.config import ensure_directories
        ...
        assert (tmp_path / "mcp_acp_extended_logs" / "audit").is_dir()
        assert (tmp_path / "mcp_acp_extended_logs" / "system").is_dir()
        assert not (tmp_path / "mcp_acp_extended_logs" / "debug").exists()
    ...
    def test_ensure_directories_creates_debug_dir_when_debug_level(
        ...
        assert (tmp_path / "mcp_acp_extended_logs" / "audit").is_dir()
        assert (tmp_path / "mcp_acp_extended_logs" / "system").is_dir()
        assert (tmp_path / "mcp_acp_extended_logs" / "debug").is_dir()
