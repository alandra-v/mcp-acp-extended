...
class TestHITLHandler:
    ...
    def test_is_supported_on_macos(self, mock_config: MagicMock) -> None:
        ...
        # Act
        with patch("mcp_acp_extended.pep.hitl.sys.platform", "darwin"):
    ...
    def test_is_not_supported_on_non_macos(self, mock_config: MagicMock) -> None:
        ...
        # Act
        with patch("mcp_acp_extended.pep.hitl.sys.platform", "linux"):
            with patch("mcp_acp_extended.pep.hitl.get_system_logger") as mock_logger:
    ...
    async def test_allow_decision_calls_next(
        ...
        with patch.object(middleware._engine, "evaluate", return_value=Decision.ALLOW):
            with patch("mcp_acp_extended.pep.middleware.get_request_id", return_value="req123"):
                with patch("mcp_acp_extended.pep.middleware.get_session_id", return_value="sess456"):
                    # Act
    ...
    async def test_hitl_user_denied_raises_permission_denied(
        ...
        with patch.object(middleware._engine, "evaluate", return_value=Decision.HITL):
            with patch.object(
                ...
                with patch.object(middleware._engine, "get_matching_rules", return_value=[]):
                    with patch(
                        "mcp_acp_extended.pep.middleware.get_request_id",
                        return_value="req123",
                    ):
                        with patch(
                            "mcp_acp_extended.pep.middleware.get_session_id",
...
class TestProtectedPaths:
    ...
    def test_is_protected_path_returns_false_for_none(self) -> None:
        ...
        from mcp_acp_extended.pdp.engine import PolicyEngine
        from mcp_acp_extended.pdp.policy import create_default_policy
    ...
    def test_evaluate_denies_protected_path(self, tmp_path: Any) -> None:
        ...
        from mcp_acp_extended.context import ActionCategory
        from mcp_acp_extended.pdp.decision import Decision
        from mcp_acp_extended.pdp.engine import PolicyEngine
        from mcp_acp_extended.pdp.policy import create_default_policy
