...
from mcp_acp_extended.pdp.engine import (
    PolicyEngine,
    _count_path_depth,
    _has_wildcards,
)
...
class TestCountPathDepth:
    ...
    def test_counts_all_segments_for_exact_path(self):
        assert _count_path_depth("/a/b/c/d") == 4

    def test_stops_at_star_in_segment(self):
        assert _count_path_depth("/a/b/file*.txt") == 2

    def test_handles_double_slashes(self):
    ...
    def test_list_returns_max_depth(self):
        patterns = ["/a/**", "/a/b/c/**", "/x/**"]
...
class TestCalculateSpecificity:
    ...
    def test_single_condition_scores_100(self, engine):
        rule = PolicyRule(
            effect="allow",
    ...
    def test_exact_tool_name_adds_10_bonus(self, engine):
        rule = PolicyRule(
            effect="allow",
    ...
    def test_two_conditions_score_200(self, engine):
        rule = PolicyRule(
            effect="allow",
    ...
    def test_path_depth_adds_bonus(self, engine):
        rule = PolicyRule(
            effect="allow",
    ...
    def test_exact_path_adds_exactness_and_depth(self, engine):
        rule = PolicyRule(
            effect="allow",
            conditions=RuleConditions(
    ...
    def test_multiple_exact_conditions(self, engine):
        rule = PolicyRule(
            effect="allow",
            conditions=RuleConditions(
    ...
    def test_subject_id_adds_exact_bonus(self, engine):
        rule = PolicyRule(
            effect="allow",
            conditions=RuleConditions(
                tool_name="read*",
